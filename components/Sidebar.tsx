...
export const Sidebar: React.FC<SidebarProps> = React.memo(({ 
  ...
  return (
    <div className={`${isCollapsed ? 'w-20' : 'w-72'} glass-dark text-white h-screen flex flex-col shadow-2xl fixed left-0 top-0 z-50 transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] border-r border-white/5`}>
      <button 
        onClick={onToggleCollapse}
        className="absolute -right-3 top-12 bg-brand-500 text-white w-7 h-7 rounded-full flex items-center justify-center shadow-glow hover:bg-brand-600 transition-all z-50 border border-white/20 hover:scale-110 active:scale-90"
      >
        <i className={`fas ${isCollapsed ? 'fa-chevron-right' : 'fa-chevron-left'} text-[10px]`}></i>
      </button>

      <div className={`p-8 mb-2 overflow-hidden whitespace-nowrap ${isCollapsed ? 'px-4 text-center' : ''}`}>
      ...
      <div className="p-6 mt-auto border-t border-white/5 bg-white/[0.02]">
        ...
        <button 
          onClick={onLogout}
          className={`w-full flex items-center text-slate-500 hover:text-red-400 transition-all rounded-xl hover:bg-red-500/10 p-4 ${isCollapsed ? 'justify-center' : 'space-x-4 px-4'}`}
        >
